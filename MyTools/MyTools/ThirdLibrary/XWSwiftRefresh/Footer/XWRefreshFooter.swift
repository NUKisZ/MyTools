...
/** 抽象类，不直接使用，用于继承后，重写*/
open class XWRefreshFooter: XWRefreshComponent {
    
    //MARK: 提供外界访问的
    /** 提示没有更多的数据 */
    open func noticeNoMoreData(){ self.state = XWRefreshState.noMoreData }
    
    /** 重置没有更多的数据（消除没有更多数据的状态） */
    open func resetNoMoreData(){  self.state = XWRefreshState.idle }
